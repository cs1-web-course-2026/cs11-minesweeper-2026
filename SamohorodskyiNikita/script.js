...
function render() {
  ...
  gameState.field.forEach((row, rowIndex) => {
    ...
    row.forEach((cell, colIndex) => {
      ...
      if (cell.state === CELL_STATE.CLOSED) cellButton.className = 'EmptyCell';
      else if (cell.state === CELL_STATE.FLAGGED) cellButton.className = 'FlagCell';
      else if (cell.state === CELL_STATE.OPENED) {
        if (cell.type === CELL_TYPE.MINE) cellButton.className = 'BombCell';
        else {
          cellButton.className = 'EmptyCellOpened';
          if (cell.neighbourMines > 0) cellButton.innerHTML = `<p class="NumCell">${cell.neighbourMines}</p>`;
        }
      }
      cellButton.addEventListener('click', () => { openCell(rowIndex, colIndex); render(); });
      cellButton.addEventListener('contextmenu', (e) => { e.preventDefault(); toggleFlag(rowIndex, colIndex); render(); });
...
function startTimer() {
  ...
  updateTimerDisplay();
  gameState.timerID = setInterval(() => { gameState.gameTime++; updateTimerDisplay(); }, 1000);
}
...
function endGame(result) {
  ...
  if (result === GAME_STATUS.LOST) gameMessageElement.textContent = 'Ви підірвалися! Гра закінчена.';
  else gameMessageElement.textContent = 'Вітаємо, ви виграли!';
  setTimeout(() => { gameMessageElement.textContent = ''; initGame(); render(); }, 3000);
}
